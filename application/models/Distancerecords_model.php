...
class Distancerecords_model extends CI_Model {
	...
	function fastquery() {
		...
		$sql = 'SELECT t1.sat, description AS sat_description, t1.distance, t2.COL_TIME_ON AS time, t2.COL_CALL AS callsign, t2.COL_GRIDSQUARE AS grid, t2.COL_MODE AS mode, t2.COL_PRIMARY_KEY AS primarykey, t2.station_gridsquare AS mygrid
			FROM (
	...
	function slowquery() {
		...
		foreach ($query->result() as $row) {
			$bindings=[];
			$subsql = 'SELECT COL_SAT_NAME AS sat, satellite.description AS sat_description, COL_TIME_ON as time, COL_CALL as callsign, COL_GRIDSQUARE as grid, station_profile.station_gridsquare AS mygrid, COL_MODE AS mode, COL_PRIMARY_KEY as primarykey
				FROM '.$this->config->item('table_name').'
				LEFT JOIN station_profile ON station_profile.station_id = '.$this->config->item('table_name').'.station_id
			...
			$subrow = $subquery->row();
			array_push($result, (object) ["sat" => $row->sat, "sat_description" => $subrow->sat_description, "distance" => $row->distance, "time" => $subrow->time, "primarykey" => $subrow->primarykey, "callsign" => $subrow->callsign, "mode" => $subrow->mode, "grid" => $subrow->grid, "mygrid" => $subrow->mygrid]);
		}
		return($result);
	}
	...
	public function sat_distances($sat){
		$this->load->model('logbooks_model');
		$logbooks_locations_array = $this->logbooks_model->list_logbook_relationships($this->session->userdata('active_station_logbook'));
		$this->db->select('*, satellite.description AS sat_description');
