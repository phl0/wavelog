...
L.Maidenhead = L.LayerGroup.extend({
	...
	_getLabel: function(lon,lat) {
	  var title_size = new Array(0, 10, 14, 16, 6.5, 8, 10, 16, 24, 36, 12, 14, 20, 36, 60, 12, 20, 36, 60, 12, 24); // Controls text size on labels
		var zoom = map.getZoom();
		...
		if (zoom > 4) {
			if(mwgrids[locator] !== undefined) {
				title = '<div><font style="color:'+this.options.color+'; font-size:'+size+'; font-weight: 900; ">' + locator;
				if (zoom > 5) {
					title += '<br />'+mwgrids[locator]+'%';
				}
				title += '</font></div>';
			}
